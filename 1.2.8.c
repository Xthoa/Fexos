...
{
	memfill(vram, x, COL8_008400,  0,     0,      x -  1, y - 1);
	return;
}
...
void apptaskfn(void);
int runcmd(char* cmd,Hcons cons)
USE_STACK(36) {
	char s[24];
...
void apptaskfn(void){
	...
	cons.taskp=task;
	cons.winui=ui;
	cons.commands=(char*)memman_alloc_4k(get_sys_memman(),1024*4);
...
void HariMain(void){
	...
	struct FIFO32 fifo;
	char s[40];
	Hsheet sht_mouse,sht_back,sht_win;
...
//	sheet_refresh(sht_win,6,26,154,46);
	appstack = (apptask->fesp = memman_alloc_4k(memman, 64 * 1024)) + 64 * 1024;
...
*/	
	apptask=alloctask("Console");
	inittask(apptask,1,2,1);
	apptask->tss.eip = apptaskfn;
	apptask->tss.esp = appstack;
	task_ready(apptask);
