...
int shellcmd(char* cmd,Hcons cons) USE_STACK(68) {
	...
	if(strcmp(cmd,"cls")==0){
		...
		Fileinfo *finfo=findfile("test.bin");
		char* pos=filepos(finfo);
		//cons_printf(cons,"%x %x %x %x",finfo,pos,pos[0],pos[1]);
		set_segmdesc(gdt+1027,finfo->len-1,(int)pos,AR_CODE32_ER);
		farcall(0,1027*8);
	...
	else cons_putline(cons,"Unknown command.");
	return 1;
}
int setcmd(char* cmd,Hcons cons){
...
void optfn(void){
	Position pos={216,48};
	msgbox("Welcome","Welcome to Fexos 1.3.1!",pos);
	while(1);
}
...
void HariMain(void){
	...
	struct FIFO32 fifo;
	char s[40];
	Hsheet sht_mouse,sht_win=NULL;
	int fifobuf[128],mx, my, i, cursor_x=8, cursor_c=1,shift=0;
