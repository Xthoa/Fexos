...
Hsheet sht_back;
Htimer task_timer;
inline struct MEMMAN* get_sys_memman(){return (struct MEMMAN*)MEMMAN_ADDR;}
inline Htask task_now(){
	return taskctl->ready[taskctl->now];
}
void init_screen8(char *vram, int x, int y){
	memfill(vram, x, back_col=42,  0,     0,      x -  1, y - 1);
	return;
}
...
void cons_putchar(Hcons cons,char i) USE_STACK(8) {
	if (i=='\b' && cons->curspos->x > 8)cons->curspos->x -= 8;
		...
		int spaces=((cons->curspos->x>>3)-1)%4;
		spaces=4-spaces;
		putfonts8_asc_sht(cons->winui->sht,cons->curspos->x,cons->curspos->y,COL8_FFFFFF,COL8_000000,"    ",spaces);
		cons->curspos->x+=spaces*8;
	}
	else if(((unsigned char)i)>=0x20){
		if(cons->curspos->x < 240){
...
int back_col;
void apptaskfn(int (*)(char*,Hcons),char*);
int setcmd(char* cmd,Hcons cons);
int runapp(char* cmd,Hcons cons){
	static char* ext[2]={".bin",".fex"};
	struct MEMMAN* memman=get_sys_memman();
	struct SEGMENT_DESCRIPTOR *gdt = (struct SEGMENT_DESCRIPTOR *) ADR_GDT;
	int fmt;
	Fileinfo* finfo=findfile(cmd);
	if(finfo==NULL){
		for(fmt=0;fmt<2;fmt++){
			strcat(cmd,ext[fmt]);
			if((finfo=findfile(cmd))!=NULL)break;
		}
	}else{
		for(fmt=0;fmt<2;fmt++){
			if(strstr(finfo->name,ext[fmt])!=NULL)break;
		}
	}
	int cs,ssx4k,ss,entry;
	if(fmt==0){
		cs=memman_alloc_4k(memman,finfo->len);
		memcpy((void*)cs,filepos(finfo),finfo->len);
		set_segmdesc(gdt+segcnt,finfo->len-1,cs,AR_CODE32_ER);
		startapp(0,(segcnt++)*8);
		memman_free_4k(memman,cs,finfo->len);
	}else if(fmt==1){
		char* pos=filepos(finfo);
		ssx4k=*(int*)pos;pos+=4;
		ssx4k*=4*1024;
		entry=*(int*)pos;pos+=4;
		cs=memman_alloc_4k(memman,finfo->len-8);
		memcpy(cs,pos,finfo->len-8);
		set_segmdesc(gdt+segcnt,finfo->len-1,cs,AR_CODE32_ER);
		ss=memman_alloc_4k(memman,ssx4k);
		set_segmdesc(gdt+segcnt+1,ssx4k-1,ss,AR_DATA32_RW);
		Htask app=alloctask(finfo->name);
		inittask(app,3,segcnt,segcnt+1);
		app->fesp=app->tss.esp=ss+ssx4k;
		app->tss.eip=entry;
		task_ready(app);
	}else return 0;
	return 1;
}
int shellcmd(char* cmd,Hcons cons) USE_STACK(68) {
	char s[48];
	struct MEMMAN* memman=get_sys_memman();
	...
	if(strcmp(cmd,"cls")==0){
		...
		sprintf(s,"total %6dMB\nused  %6dMB\nfree  %6dMB",memtotal>>20,(memtotal-total)>>20,total>>20);
		cons_putline(cons,s);
	}else if(strcmp(cmd,"nothing")==0)total=total;
	else if(strncmp(cmd,"echo ",5)==0)cons_printf(cons,"%s\n",cmd+5);
	else if(strcmp(cmd,"cmd")==0)create_console(shellcmd,"Console");
	else if(strcmp(cmd,"settings")==0)create_console(setcmd,"Settings");
	else if(strcmp(cmd,"dir")==0){
		sprintf(s,"All %d files",fs.filenum);
		cons_putline(cons,s);
		int i;
		for(i=0;i<fs.filenum;i++){
			Fileinfo* finfo=&(((Fileinfo*)(fs.start))[i]);
			cons_putline(cons,finfo->name);
		}
	}else if(strcmp(cmd,"ls")==0){
		cons_printf(cons,"All %d files\n",fs.filenum);
		int i;
		for(i=0;i<fs.filenum;i++){
			Fileinfo* finfo=&(((Fileinfo*)(fs.start))[i]);
			cons_printf(cons,"%4d %2d ",finfo->len,finfo->flag);
			cons_putline(cons,finfo->name);
		}
	}else if(strncmp(cmd,"type ",5)==0){
		Fileinfo* finfo=findfile(cmd+5);
		if(finfo==NULL){
			cons_putline(cons,"File Not Found.");
			return 1;
		}
		cons_putstr(cons,filepos(finfo));
	}
	else if(strcmp(cmd,"exit")==0)return 0;
	else if(runapp(cmd,cons)==0)cons_putline(cons,"Unknown command.");
	return 1;
}
int setcmd(char* cmd,Hcons cons){
	if(strncmp(cmd,"bg",2)==0){
		if(strcmp(cmd+3,"")==0)cons_printf(cons,"Bgcolor: %d\n",back_col);
		else{
			cons_printf(cons,"Bgcolor: %d\n",back_col=atoi(cmd+3));
			memfill(sht_back->buf,sht_back->bxsize,back_col,0,0,sht_back->bxsize-1,sht_back->bysize-1);
...
void optfn(void){
	Position pos={216,48};
	msgbox("Welcome","Welcome to Fexos 1.3.2!",pos);
	while(1);
}
void apptaskfn(int (*runcmd)(char*,Hcons),char* title){
	Console cons;
	Fifo fifo;
	Htask task=task_now();
	cons.cmdfunc=runcmd;
	...
	UI* ui=create_window(&fifo,winpos,title);
			...
			if(i==1){
				...
				if (i=='\b' && curspos.x > 16) { /* \b */
					...
					*(up++)=0;
					...
					int spaces=((curspos.x>>3)-2)%4;
					spaces=4-spaces;
					putfonts8_asc_sht(ui->sht,curspos.x,curspos.y,COL8_FFFFFF,COL8_000000,"    ",spaces);
					curspos.x+=spaces*8;
					for(;spaces>0;spaces--)*(up++)=0x20;
				}
				else if(curspos.x < 240 && ((unsigned char)i)>=0x20){
					putfonts8_asc_sht(ui->sht,curspos.x,curspos.y,COL8_FFFFFF,COL8_000000,(char*)&i,1);
					curspos.x += 8;
					*(up++)=i;
				}
			}
	...
	memman_free_4k(memman,(unsigned int)cons.commands,4*1024);
	sheet_free(ui->sht);
	freeui(ui);
...
void HariMain(void){
	...
	struct FIFO32 fifo;
	char s[40];
	Hsheet sht_mouse,winpt=NULL;
	int fifobuf[128],mx, my, i,shift=0,mvx=0,mvy=0;
	struct MOUSE_DEC mdec;
	struct MEMMAN *memman = (struct MEMMAN *) MEMMAN_ADDR;
	unsigned char buf_mouse[256],*buf_back;
	Htask systask;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0)io_stihlt();
			...
			if (256 <= i && i <= 511) {
				...
				if(shift==0)s[0] = keytable[i - 256];
				else if((shift&1)==1)s[0]=Keytable[i-256];
				else s[0]=keyTable[i-256];
				fifo32_put(shtctl->sheets[shtctl->top-1]->fifop,s[0]);
			} else if (512 <= i && i <= 767) {
				if (mouse_decode(&mdec, i - 512) != 0) {
					...
					Hsheet sht;
					int ht,x,y;
					for(sht=NULL,x=y=0,ht=shtctl->top-1;ht>0;ht--){
					...
					if ((mdec.btn & 0x01) != 0){
						fifo32_put(sht->fifop,MSG_MOUSEL);
						if(mvx<0){
					...
					if ((mdec.btn & 0x02) != 0)fifo32_put(sht->fifop,MSG_MOUSER);//mouse right
					if ((mdec.btn & 0x04) != 0)fifo32_put(sht->fifop,MSG_MOUSEC);//mouse middle
				}
			}
		}
	}
...
FS fs;
void initfs(){
	char* fat=(char*)(ADR_DISKIMG+0x2600);
	short clustno=*(short*)(fat+0x3a);
	char* pos=(char*)(ADR_DISKIMG+0x3e00+clustno*512);
	fs.filenum=*(int*)pos;
	fs.start=(Fileinfo*)(pos+32);
}
Fileinfo* findfile(char* name){
	int i;
	for(i=0;i<fs.filenum;i++){
		if(strcmp((fs.start)[i].name,name)==0){
			return &((fs.start)[i]);
		}
	}
	return NULL;
}
char* filepos(Fileinfo* finfo){
	return (char*)(((char*)fs.start)+finfo->pos);
}
/******************
 *       UI       *
 ******************/
UI* uilst;
void initui(){
	uilst=(UI*)memman_alloc_4k(get_sys_memman(),sizeof(UI)*2048);//32*2048=64k
}
UI* allocui(Hsheet sht,Hfifo fifop,Area area){
	...
	for(i=0;i<MAX_UI_COUNT;i++){
		if(uilst[i].flag==0){
			...
			ui->pos_sht=area;
			ui->sht=sht;
			ui->childlst=(UI**)memman_alloc(get_sys_memman(),128);
			ui->childcnt=0;
			ui->flag=1;//Using
			return ui;
		}
...
UI* create_window(Hfifo fifop,Position xylen,char* title){
	...
	char* buf=(char*)memman_alloc_4k(get_sys_memman(),xylen.x*xylen.y);
...
void create_console(void* cmdfunc,char* title){
	...
	inittask(task,1,2,1);
	task->tss.eip=(int)apptaskfn;
	task->tss.esp=(task->fesp=memman_alloc_4k(get_sys_memman(),64*1024))+64*1024-12;
	*(int*)(task->tss.esp+4)=(int)cmdfunc;
	*(int*)(task->tss.esp+8)=(int)title;
	task_ready(task);
}
...
void init_gdtidt(void){
	...
	set_gatedesc(idt + 0x32, (int) asm_int32api, 2 * 8, AR_INTGATE32);
	return;
}
...
void sheet_refreshsub( int vx0, int vy0, int vx1, int vy1, int h0, int h1){	struct SHTCTL *ctl=shtctl;
	int h, bx, by, vx, vy, bx0, by0, bx1, by1;
...
void init_pit(void){
	...
	struct TIMER *t = timer_alloc(); /* 一つもらってくる */
...
Htask initmt(struct MEMMAN *memman) USE_STACK(12) {
	...
	for (i = 0; i < MAX_TASKS; i++) {
		...
		set_segmdesc(gdt + 3 + i, 127, (int) &taskctl->tasks0[i], AR_TSS32);
	}
